...
class YOLOv8Inference:
    ...
    def postprocess(self, output, orig_img):
        """
        Postprocess detections
        Args:
            output: Model output tensor containing [x1, y1, x2, y2, score, class]
        ...
        # Output shape is [batch, num_dets, 6] where last dim is [x1, y1, x2, y2, score, class]
        for detection in output[0]:  # Process first batch only
            ...
            if score > self.conf_thres:
                ...
                x2 = min(img_width, int(x2 * scale_x))
                y2 = min(img_height, int(y2 * scale_y))
    ...
    def __call__(self, img):
        ...
        # Postprocess detections
        detections = self.postprocess(outputs[0], img)
...
def process_video(model, video_source, output_path=None, show_display=True):
    ...
    Args:
        model: YOLOv8Inference instance
